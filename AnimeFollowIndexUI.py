...
from AnimeFollowRightclickUI import FollowRightclick

class FollowIndexWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.followlist.setGeometry(17,280,self.pix.width()-34,self.pix.height()-280-50)
        self.followlist.setStyleSheet('#FollowListWidget{background:transparent;}')
    ...
    def ShowData(self):
        follow = Follow()
        self.infos = follow.FollowInAnime()
        self.followlist.clear()
        if self.infos:
            for info in self.infos:
                self.followlist.addItem(QListWidgetItem(QIcon('./source/pic/bili.png'), info))
    ...
    def DetailBt(self):
        if self.detail_show == False:
            info = self.GetDetail(self.followlist.currentItem().text())
            self.followlist.index = self.followlist.currentIndex()
            self.detail.setInfo(info)
            self.detail.show()
            self.detail_show = True
        elif self.detail_show == True and self.followlist.index != self.followlist.currentIndex():
            self.detail.close()
            self.DelDetail()
            info = self.GetDetail(self.followlist.currentItem().text())
            self.followlist.index = self.followlist.currentIndex()
